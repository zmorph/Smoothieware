    ...
    if ( update_counts % 20 == 0 ) {
        ...
        if (this->panel->lcd->hasGraphics()) {
            ...
            if (this->hotendtarget > 0)
                this->panel->lcd->bltGlyph(x_start + 8  , y_start, 20, 19, icons, 15, 0, 0);

            if (this->hotend2target >0)
                this->panel->lcd->bltGlyph(x_start + 24 , y_start, 20, 19, icons, 15, 24, 0);

            if (this->bedtarget > 0)
                this->panel->lcd->bltGlyph(x_start + 40 , y_start, 23, 19, icons, 15, 64, 0);

            if(this->fan_state)
                this->panel->lcd->bltGlyph(x_start + 96 , y_start, 23, 19, icons, 15, 96, 0);
        }
    }
}
    ...
    switch ( line ) {
        case 0: {
            ...
            if(hotend2temp != -2) { sprintf(hotend2temp_text, "Q=%03d/%03dc", this->hotend2temp,  this->hotend2target); }
                this->panel->lcd->printf("%s%s", hotendtemp_text, hotend2temp_text); break; }
